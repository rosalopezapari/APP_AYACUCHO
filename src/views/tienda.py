...
class VentanaTienda:
    def __init__(self, root):
        self.root = root
        self.root.title(_("Qory Ayacucho - Tiendas y Artesanías"))
        ...
        columns = ("nombre", "provincia", "tipo", "precio_min", "precio_max", "especialidad")
        ...
        self.tree.column("especialidad", width=160)
    ...
    def cargar_filtros(self):
        opciones = [_("Todas")] + Tienda.tipos() + Tienda.provincias()
    ...
    def _get_filtro(self):
        ...
        if not sel or sel == _("Todas"):
            return None, None
        if sel in Tienda.tipos():
            return "tipo", sel
        if sel in Tienda.provincias():
    ...
    def cargar(self):
        ...
        if tipo == "tipo":
            datos = Tienda.listar_por_tipo(valor)
        elif tipo == "provincia":
            datos = Tienda.listar_por_provincia(valor)
        else:
            datos = Tienda.listar_todos()

        for r in datos:
            self.tree.insert("", tk.END, iid=r["id_tienda"], values=(
                r["nombre"], r["provincia"], r["tipo"],
    ...
    def agregar(self):
        DialogoTienda(self.root, guardar_callback=lambda: self.cargar())

    def editar(self):
        id_t = self._selected()
        if not id_t:
            messagebox.showerror(_("Error"), _("Selecciona una tienda"))
            return
        t = Tienda.obtener(id_t)
        if t:
            DialogoTienda(self.root, tienda=t, guardar_callback=lambda: self.cargar())
    ...
    def eliminar(self):
        ...
        if messagebox.askyesno(_("Confirmar"), _("¿Eliminar %s?") % t['nombre']):
            exito, msg = Tienda.eliminar(id_t)
            if exito:
                self.cargar()
            messagebox.showinfo(_("Éxito") if exito else _("Error"), msg)


class DialogoTienda(tk.Toplevel):
    def __init__(self, parent, tienda=None, guardar_callback=None):
        super().__init__(parent)
        self.tienda = tienda
        self.guardar_callback = guardar_callback
        titulo = _("Editar Tienda") if tienda else _("Agregar Tienda")
        self.title(titulo)
        ...
        self.provincia_combo = ttk.Combobox(frame, values=Tienda.provincias(), state="readonly", width=37)
        self.provincia_combo.grid(row=1, column=1, pady=5)

        ttk.Label(frame, text=_("Tipo:")).grid(row=2, column=0, sticky=tk.W, pady=5)
        self.tipo_combo = ttk.Combobox(frame, values=Tienda.tipos(), state="readonly", width=37)
    ...
    def cargar_datos(self):
        ...
        self.provincia_combo.set(t["provincia"])
        self.tipo_combo.set(t["tipo"])
        self.min_entry.insert(0, str(t["precio_min"]))
        self.max_entry.insert(0, str(t["precio_max"]))
        self.especialidad_entry.insert(0, t["especialidad"] or "")
    ...
    def guardar(self):
        ...
        if self.tienda:
            exito, msg = Tienda.actualizar(
                self.tienda["id_tienda"], provincia, tipo, nombre, precio_min, precio_max, especialidad,
            )
        else:
            exito, msg = Tienda.agregar(provincia, tipo, nombre, precio_min, precio_max, especialidad)
